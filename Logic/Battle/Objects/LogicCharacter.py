...
class LogicCharacter(BitStream):
    def __init__(self, device):
        self.device = device
    def encodeSpecial(player, stream, heroes, index):
        LogicGameObjectServer.encode(stream,heroes["objectInfos"])
        stream.writePositiveInt(0, 3)
        stream.writePositiveInt(1, 13) # current hp
        stream.writePositiveInt(1, 13) # max hp
    ...
    def encode(player, stream, heroes, index):
        ...
        stream.writePositiveInt(heroes["currentHP"], 13) # current hp
        stream.writePositiveInt(heroes["maximumHP"], 13) # max hp
        stream.writePositiveInt(heroes["itemsAmount"], 6) # items amount (here its bounty stars)
        stream.writeBoolean(heroes["hasBall"])
        ...
        for index, x in heroes["damagesArray"].items():
            stream.writePositiveInt(heroes["damagesArray"][index]["damage"], 15) # damages dealt
        # skills array
        # skills
