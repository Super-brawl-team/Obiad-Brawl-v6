...
class LogicGameObjectManagerServer(BitStream):
    ...
    def encode(self, ownObjectGlobalId, playerIndex, teamIndex, gamemodeVariation, player, battleInfo):
        ...
        self.writeBoolean(battleInfo["unknownBoolean"])
        self.writeBoolean(battleInfo["unknownBoolean1"])
        self.writeBoolean(battleInfo["unknownBoolean2"])
        self.writeBoolean(battleInfo["unknownBoolean3"])
        ...
        for index, ulti in battleInfo["ultiArray"].items():
            ...
            if int(index) == 1:
                self.writePositiveInt(battleInfo["ultiArray"][index]["ultiCharge"], 12) # ulti charge (only for your player) 1000 is full ulti
        ...
        for index, kill in battleInfo["killArray"].items():
            ...
            self.writePositiveInt(battleInfo["killArray"][index]["score"], 7)
            self.writePositiveInt(len(battleInfo["killArray"][index]["entry"]), 4)
        ...
        for index, array in battleInfo["gameObjects"]["gameObjects"]["characters"].items():
            LogicCharacter.encodeSpecial(player, self, battleInfo["gameObjects"]["gameObjects"]["characters"][index], int(index))
